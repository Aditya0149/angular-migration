(function (angular) {
    ...
    angular.module('cloudlex.workflow')
        .controller('addWorkflowCtrl', addWorkflowCtrl);

    addWorkflowCtrl.$inject = ['addWorkflowDatalayer', 'notification-service', 'workFlowTemplateDataService'];
    function addWorkflowCtrl(addWorkflowDatalayer, notificationService, workFlowTemplateDataService) {
        ...
        var workflowData = workFlowTemplateDataService.getSelectedTemplateData();
        var workflowList = workFlowTemplateDataService.getTemplatelist();
        ...
        (function () {
            ...
            //JSON.parse(userData.workflow_dates)
        ...
        function formatData(editData) {
            ...
            vm.workflowInfo.SOL = angular.isDefined(editData.matter_event[0]) ? utils.isNotEmptyVal(editData.matter_event[0].code) ? "1" : 0 : 0;
        ...
        function save(workflowData) {
            ...
            if (vm.mode === 'Clone') {
                var promise = addWorkflowDatalayer.cloneWorkflow(postDataObj);
            } else {
                var promise = vm.mode === 'Edit' ? addWorkflowDatalayer.editWorkflow(postDataObj, vm.workflowInfo.workflow_id) : addWorkflowDatalayer.addWorkflow(postDataObj);
            }
        ...
        function jsonData(workflowData) {
            ...
            if (workflowData.SOL == 1) {
                ...
                msd.code = 'SOL'
                msd.id = '1';
                workflowData.matter_event.push(msd);
            }
            ...
            var postObj = {};
            // postObj.matter_specific_dates={};
            postObj.matter_specific_dates = workflowData.matterSpecificDates;
            postObj.matter_event = workflowData.matter_event;
...
(function (angular) {
    ...
    angular.module('cloudlex.workflow')
        .factory('addWorkflowDatalayer', addWorkflowDatalayer);

    addWorkflowDatalayer.$inject = ['$http', 'globalConstants'];
    function addWorkflowDatalayer($http, globalConstants) {

        var urls = {
            editWorkflow: globalConstants.javaWebServiceBaseV4 + 'workflow_template/',
            addWorkflow: globalConstants.javaWebServiceBaseV4 + 'workflow_template',
            cloneWorkflow: globalConstants.javaWebServiceBaseV4 + 'workflow_template/clone',
        };

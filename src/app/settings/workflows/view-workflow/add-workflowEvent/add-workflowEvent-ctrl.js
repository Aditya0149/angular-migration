(function (angular) {
    ...
    addEventController.$inject = ['$scope', 'dateUtils', 'addEventDatalayer', 'workflowHelper', 'globalConstants', 'notification-service', 'masterData', 'workFlowTemplateDataService', 'workFlowDatalayer', 'practiceAndBillingDataLayer', '$timeout'];
    function addEventController($scope, dateUtils, addEventDatalayer, workflowHelper, globalConstants, notificationService, masterData, workFlowTemplateDataService, workFlowDatalayer, practiceAndBillingDataLayer, $timeout) {
        ...
        var eventData = workFlowTemplateDataService.getSelectedEventData();
        ...
        function init() {
            ...
            vm.dateFieldValue = 0;
            vm.eventInfo.before_after = "Before";
        ...
        function formatDates(editData) {
            ...
            vm.eventInfo.SOL = angular.isDefined(editData.matter_event[0]) ? utils.isNotEmptyVal(editData.matter_event[0].code) ? "1" : 0 : 0;

            _.forEach(editData.matter_specific_dates, function (event) {
                if (event.code == 'DOI') {
        ...
        function afterDates(param) {
            vm.dateFieldValue = param;
            vm.eventInfo.before_after = "After";
        }
        ...
        function formatData(eventData) {
            ...
            vm.dateFieldValue = copyEvent.before_after == "Before" ? 0 : 1;
            ...
            if (utils.isNotEmptyVal(copyEvent.assigned_users_role)) {
                copyEvent.remind_users_temp = [];
                if (copyEvent.assigned_users_role && copyEvent.assigned_users_role.length > 0) {
                    _.forEach(JSON.parse(copyEvent.assigned_users_role), function (taskid, taskindex) {
                        copyEvent.remind_users_temp.push((taskid));
                    });
                }
                copyEvent.roleid = copyEvent.remind_users_temp;
            }
        ...
        function addEvent(event) {
            ...
            addEventDatalayer.addEvent(postEventData)
                .then(function (response) {
        ...
        function editEvent(event) {
            ...
            // } 
            var postEventData = getPostEventData(event);
            addEventDatalayer.editEvent(postEventData)
                .then(function (response) {
        ...
        function getPostEventData(event) {
            ...
            copyEvent.title = (copyEvent.title) ? copyEvent.title : "";
            copyEvent.title = (selectEvent.LabelId == "19") || (selectEvent.LabelId == "100") || (selectEvent.LabelId == "32") ? copyEvent.title : selectEvent.Name;
            return copyEvent;
        }
...
(function (angular) {
    ...
    angular.module('cloudlex.workflow')
        .factory('addEventDatalayer', addEventDatalayer);

    addEventDatalayer.$inject = ['$http', 'globalConstants'];
    function addEventDatalayer($http, globalConstants) {

        var urls = {
            editEvent: globalConstants.javaWebServiceBaseV4 + 'workflow_template/event/',
            addEvent: globalConstants.javaWebServiceBaseV4 + 'workflow_template/event',

(function (angular) {
    ...
    angular.module('cloudlex.workflow').
        controller('applyWorkflowTaskCtrl', applyWorkflowTaskCtrl);
    applyWorkflowTaskCtrl.$inject = ['masterData', '$timeout', 'notification-service', '$scope', 'applyTaskDatalayer', 'applyWorkflowDatalayer', '$stateParams', 'globalConstants', 'workFlowDataService'];


    function applyWorkflowTaskCtrl(masterData, $timeout, notificationService, $scope, applyTaskDatalayer, applyWorkflowDatalayer, $stateParams, globalConstants, workFlowDataService) {
        ...
        function start() {
            ...
            var selectedApplyTaskData = localStorage.getItem("selectedApplyTaskData");
            vm.taskInfo = JSON.parse(selectedApplyTaskData);
            if ((utils.isNotEmptyVal(selectedApplyTaskData)) && vm.taskInfo.length > 0) {
                //vm.taskInfo = JSON.parse(selectedApplyTaskData);
        ...
        function getTasksEventsList() {
            ...
            applyWorkflowDatalayer.getTasksEventsList(workflowID, vm.matterId)
                .then(function (response) {
                    ...
                    // vm.selectedTasks = angular.copy(response.data.workflow.workflow_task);
                });
        }
        ...
        function goToEvents(setApllyTaskData) {
            ...
            if (vm.validUserIDs) {
                ...
                localStorage.setItem("selectedApplyTaskData", JSON.stringify(setApllyTaskData));
                _.forEach(setApllyTaskData, function (item) {
                    item.user_ids = angular.isDefined(item.user_id) ? item.user_id : '';
                ...
                applyTaskDatalayer.setApllyTaskData(setApllyTaskData);
            } else {
...
(function (angular) {
    ...
    angular.module('cloudlex.workflow')
        .factory('applyTaskDatalayer', applyTaskDatalayer);


    applyTaskDatalayer.$inject = ['$http', 'globalConstants'];
    function applyTaskDatalayer($http, globalConstants) {

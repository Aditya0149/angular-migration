(function (angular) {
    ...
    angular.module('cloudlex.workflow').
        controller('applyWorkflowCtrl', applyWorkflowCtrl);
    applyWorkflowCtrl.$inject = ['masterData', 'notification-service', '$scope', 'applyWorkflowDatalayer', 'mattWorkFlowHelper', '$stateParams'];


    function applyWorkflowCtrl(masterData, notificationService, $scope, applyWorkflowDatalayer, mattWorkFlowHelper, $stateParams) {
        ...
        (function () {
            ...
            var applyWorkInfoData = localStorage.getItem("applyWorkflowInfo");
            if (utils.isNotEmptyVal(applyWorkInfoData)) {
                ...
                vm.applyWorkInfo.matter.date_of_incidence = (utils.isEmptyVal(vm.applyWorkInfo.matter.date_of_incidence)) || vm.applyWorkInfo.matter.date_of_incidence == 0  ? "" : utils.getUTCTimeStamp(vm.applyWorkInfo.matter.date_of_incidence);
                vm.applyWorkInfo.matter.date_of_incidence = (utils.isEmptyVal(vm.applyWorkInfo.matter.date_of_incidence)) ? "" : moment.unix(vm.applyWorkInfo.matter.date_of_incidence).utc().format('MM/DD/YYYY');

                vm.applyWorkInfo.matter.intake_date = (utils.isEmptyVal(vm.applyWorkInfo.matter.intake_date)) || vm.applyWorkInfo.matter.intake_date == 0 ? "" : utils.getUTCTimeStamp(vm.applyWorkInfo.matter.intake_date);
                vm.applyWorkInfo.matter.intake_date = (utils.isEmptyVal(vm.applyWorkInfo.matter.intake_date)) ? "" : moment.unix(vm.applyWorkInfo.matter.intake_date).utc().format('MM/DD/YYYY');

                if (angular.isUndefined(vm.applyWorkInfo.sol)) {
                    vm.applyWorkInfo.sol = (utils.isEmptyVal(vm.applyWorkInfo.sol)) ? "" : utils.getUTCTimeStamp(vm.applyWorkInfo.sol[0]);
        ...
        function getVerifyWorkflowData(workFlowInfo) {
            applyWorkflowDatalayer.getWorkflowList(vm.matterId)
                .then(function (response) {

                    response.data.Sol = (utils.isEmptyVal(response.data.sol)) ? "" : moment.unix(response.data.sol[0]).utc().format('MM/DD/YYYY');
                    if (utils.isNotEmptyVal(response.data.matter.date_of_incidence) && response.data.matter.date_of_incidence != 0 ) {
                        vm.dissableDoi = true;
                    }
                    if (utils.isNotEmptyVal(response.data.matter.intake_date) && response.data.matter.intake_date != 0) {
        ...
        function resetWorkflowData(workFlowInfo) {
            applyWorkflowDatalayer.getWorkflowList(vm.matterId)
                .then(function (response) {
                    ...
                    response.data.sol = (utils.isEmptyVal(response.data.sol)) ? "" : moment.unix(response.data.sol[0]).utc().format('MM/DD/YYYY');
                    if (utils.isEmptyVal(response.data.matter.date_of_incidence) || response.data.matter.date_of_incidence == 0) {
                        vm.applyWorkInfo.matter.date_of_incidence = '';
                    }
                    if (utils.isEmptyVal(response.data.matter.intake_date) || response.data.matter.intake_date == 0) {
                        vm.applyWorkInfo.matter.intake_date = '';
                    }
        ...
        function getWorkflowList() {
            applyWorkflowDatalayer.getWorkflowList(vm.matterId)
                .then(function (response) {
                    ...
                    vm.applyWorkInfo.matter.date_of_incidence = (utils.isEmptyVal(vm.applyWorkInfo.matter.date_of_incidence)) || vm.applyWorkInfo.matter.date_of_incidence == 0 ? "" : moment.unix(vm.applyWorkInfo.matter.date_of_incidence).utc().format('MM/DD/YYYY');
                    vm.applyWorkInfo.matter.intake_date = (utils.isEmptyVal(vm.applyWorkInfo.matter.intake_date) || vm.applyWorkInfo.matter.intake_date == 0) ? "" : moment.unix(vm.applyWorkInfo.matter.intake_date).utc().format('MM/DD/YYYY');
                    vm.applyWorkInfo.matter.matter_created_date = (utils.isEmptyVal(vm.applyWorkInfo.matter.matter_created_date)) ? "" : moment.unix(vm.applyWorkInfo.matter.matter_created_date).utc().format('MM/DD/YYYY');
                    vm.applyWorkInfo.Sol = (utils.isEmptyVal(vm.applyWorkInfo.sol)) ? "" : moment.unix(vm.applyWorkInfo.sol[0]).utc().format('MM/DD/YYYY');


                    if (utils.isNotEmptyVal(vm.applyWorkInfo.matter.date_of_incidence)) {
                        vm.dissableDoi = true;
                    }
                    if (utils.isNotEmptyVal(vm.applyWorkInfo.matter.intake_date)) {
                        vm.dissableDoin = true;
                    }
        ...
        function formatDates(editData) {
            ...
            vm.Info.SOL = angular.isDefined(editData.matter_event[0]) ? utils.isNotEmptyVal(editData.matter_event[0].code) ? 1 : 0 : 0;
        ...
        function goNext(applyWorkflowInfo) {
			//Bug#9335
            // if (utils.isNotEmptyVal(applyWorkflowInfo.matter.intake_date)) {
            //     var matter.intake_date = new Date(applyWorkflowInfo.matter.intake_date) ;
            //     var created_date = new Date(applyWorkflowInfo.matter.matter_created_date);
            //     if (created_date < matter.intake_date) {
            //         notificationService.error("Intake Date should be less than Created Date");
            //         return;
            //     }
            // }

            localStorage.setItem("applyWorkflowInfo", JSON.stringify(applyWorkflowInfo));
            vm.goToTask = true;
            applyWorkflowDatalayer.setApllyWorkflowData(applyWorkflowInfo);
            // alert('in controller');
        }
...
(function (angular) {
    ...
    angular.module('cloudlex.workflow')
        .factory('applyWorkflowDatalayer', applyWorkflowDatalayer);


    applyWorkflowDatalayer.$inject = ['$http', 'globalConstants'];
    function applyWorkflowDatalayer($http, globalConstants) {

        var selectedWorkflowData = {};

        var urls = {
            workflowList: globalConstants.javaWebServiceBaseV4 + 'workflow_apply',
            TaskEventsList: globalConstants.javaWebServiceBaseV4 + 'workflow_apply',
        ...
        function _getWorkflowList(matterId) {
            var data = { 'matterId': matterId.toString() };
            var url = urls.workflowList + '?' + getParams(data);;
            return $http.get(url);
        }

        function _getTasksEventsList(selWorkflowId, matterId) {
            var data = { 'workflowId': selWorkflowId.toString(), 'matterId': matterId };

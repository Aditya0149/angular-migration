(function (angular) {
    ...
    angular.module('cloudlex.workflow').
        controller('applyWorkflowEventCtrl', applyWorkflowEventCtrl);
    applyWorkflowEventCtrl.$inject = ['$timeout', 'globalConstants', 'notification-service', 'applyTaskDatalayer', 'applyEventDatalayer', 'applyWorkflowDatalayer', '$stateParams', 'workFlowDataService'];


    function applyWorkflowEventCtrl($timeout, globalConstants, notificationService, applyTaskDatalayer, applyEventDatalayer, applyWorkflowDatalayer, $stateParams, workFlowDataService) {
        ...
        function start() {
            vm.seldWorkflowData = applyWorkflowDatalayer.getApllyWorkflowData();
            vm.seldWorkTaskData = applyTaskDatalayer.getApllyTaskData();
            var selectedApplyEventData = localStorage.getItem("selectedApplyEventData");
            vm.eventInfo = JSON.parse(selectedApplyEventData);
            if ((utils.isNotEmptyVal(selectedApplyEventData)) && vm.eventInfo.length > 0) {
        ...
        function getAssignedToList(workflowID, matterID) {
            applyWorkflowDatalayer.getTasksEventsList(workflowID, matterID)
                .then(function (response) {
                    ...
                    _.forEach(assignedTo.matter_assigned_users, function (user) {
                        user.isFirmUser = 0;
                        vm.assignedTo.push(user);
                    });
                });
        ...
        function getTasksEventsList() {
            ...
            applyWorkflowDatalayer.getTasksEventsList(workflowID, vm.matterId)
                .then(function (response) {
                    var assignedTo = response.data;
                    _.forEach(assignedTo.all_firm_users, function (user) {
                        user.isFirmUser = 1;
                    ...
                    _.forEach(vm.eventInfo, function (item) {
                        ...
                        if (item.assigned_users_role && item.assigned_users_role.length > 0) {
                            var usr = [];
                            _.forEach(item.assigned_users_role, function (roleId) {
                                ...
                                if (roleDetail) {
                                    ...
                                    if (usrDetails && usrDetails.roleUsers.length > 0) {
                                        _.forEach(usrDetails.roleUsers, function (individualuser) {
                                            individualuser.full_name = individualuser.name
                                            usr.push(parseInt(individualuser.uid));
                                        });

                                    }
                                }
                            });
                            item.assigned_to = _.uniq(usr);
                        }
                    });
                });
        }
        ...
        function calDueDate(events) {
            _.forEach(events, function (event) {
                ...
                var doin = vm.seldWorkflowData.matter.intake_date;
                var doi = vm.seldWorkflowData.matter.date_of_incidence;
                var mcd = vm.seldWorkflowData.matter.matter_created_date;
        ...
        function setStartEndDateTime(event) {
            if (event.all_day == 0) {
                ...
                event.start = utils.getUTCTimeStamp(event.duedate);
                event.end = utils.getUTCTimeStampEndDay(event.duedate);
            }
        }
        ...
        function setEventDates(eventduedate, event) {
            ...
            event.start = event.all_day == '1' ? formTimestamp(event.utcstart, event.start_time, true, 'start') :
                formTimestamp(event.utcstart, event.start_time, false);

            event.end = event.all_day == '1' ? formTimestamp(event.utcend, event.end_time, true, 'end') :
        ...
        function goToTasks(applyEventData) {
            ...
            localStorage.setItem("selectedApplyEventData", JSON.stringify(applyEventData));
            vm.goToTask = true
            vm.viewapplyworkflow = true;
            vm.goToEvent = false;
        }
        ...
        function apply(applyEventData) {
            ...
            _.forEach(applyEventInfo, function (item) {
                ...
                item.duedate = (utils.isEmptyVal(item.duedate)) ? "" : utils.getUTCTimeStamp(item.duedate);
                item.custom_reminder = (utils.isEmptyVal(item.custom_reminder)) ? "" : utils.getUTCTimeStamp(item.custom_reminder);
                item.user_ids = angular.isDefined(item.assigned_to) && item.assigned_to != null ? item.assigned_to : [];
            ...
            applyEventDatalayer.setApplyEventData(applyEventInfo);
            var seldWorkEventData = applyEventDatalayer.getApplyEventData();
            ...
            applyEventDatalayer.applyWorkflow(postWorkFlowData)
                .then(function (response) {
        ...
        function postData(applyworkflowData) {
            var postDataObj = {};
            postDataObj.matter = applyworkflowData.workflowDetails.matter;
            ...
            postDataObj.matter.date_of_incidence = (utils.isEmptyVal(applyworkflowData.workflowDetails.matter.date_of_incidence)) ? "" : utils.getUTCTimeStamp(applyworkflowData.workflowDetails.matter.date_of_incidence);
            postDataObj.matter.intake_date = (utils.isEmptyVal(applyworkflowData.workflowDetails.matter.intake_date)) ? "" : utils.getUTCTimeStamp(applyworkflowData.workflowDetails.matter.intake_date);
            postDataObj.matter.matter_created_date = (utils.isEmptyVal(applyworkflowData.workflowDetails.matter.matter_created_date)) ? "" : utils.getUTCTimeStamp(applyworkflowData.workflowDetails.matter.matter_created_date);
            postDataObj.matter.matter_id = vm.matterId;
        ...
        function groupAssignedToUsers(user) {
            return applyWorkflowDatalayer.groupByUserType(user);
        }
...
(function (angular) {
    ...
    angular.module('cloudlex.workflow')
        .factory('applyEventDatalayer', applyEventDatalayer);


    applyEventDatalayer.$inject = ['$http', 'globalConstants'];
    function applyEventDatalayer($http, globalConstants) {

        var selectedEventData = {};
        var urls = {
            TaskEventsList: globalConstants.javaWebServiceBaseV4 + 'workflow_apply',
            applyWorkFlow: globalConstants.javaWebServiceBaseV4 + 'workflow_apply',
        ...
        function _getTasksEventsList(selWorkflowId) {
            var data = { 'wid': selWorkflowId.toString() };

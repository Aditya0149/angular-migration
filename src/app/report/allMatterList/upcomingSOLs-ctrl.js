(function () {
    ...
    angular.module('cloudlex.report').controller('UpcomingSOLsCtrl', ['reportFactory', 'upcomingSOLsReportHelper', '$modal',
        function (reportFactory, upcomingSOLsReportHelper, $modal) {
            ...
            self.init = function () {
                var viewModel = sessionStorage.getItem("upcomingSOLViewModel");
                var selectionModel = sessionStorage.getItem("upcomingSOLSelectionModel");
                if (utils.isNotEmptyVal(viewModel) || utils.isNotEmptyVal(selectionModel)) {
                    try {
                        ...
                        self.tags = upcomingSOLsReportHelper.generateTags(self.viewModel.filters);
                ...
                self.clxGridOptions = {
                    headers: upcomingSOLsReportHelper.getGridHeaders(),
                    selectedItems: []
                };
            }
            ...
            self.filterSOLReport = function () {
                ...
                modalInstance.result.then(function (filterObj) {
                    ...
                    self.tags = upcomingSOLsReportHelper.generateTags(filterObj.filter);
                    self.showPager = true;
            ...
            function persistFilters() {
                ...
                viewModel.matters = [];
                sessionStorage.setItem("upcomingSOLViewModel", JSON.stringify(viewModel));
                sessionStorage.setItem("upcomingSOLSelectionModel", JSON.stringify(selectionModel));
            }
            ...
            function applyFilterLazyLoading() {
                ...
                getNextMatterList(self.viewModel.filters);
            };
            ...
            function getMatterList(getAll, info) {
                ...
                if (utils.isEmptyVal(self.viewModel.filters.s) && utils.isEmptyVal(self.viewModel.filters.e)) {
                    ...
                    self.viewModel.filters.e = end;
                    self.tags = upcomingSOLsReportHelper.generateTags(self.viewModel.filters);//Bug#5619
                }
                if (info == 'calDays') {
                ...
                var filterObj = upcomingSOLsReportHelper.getFiltersObj(self.viewModel.filters, getAll);
                persistFilters();
                var promesa = reportFactory.getMatterstats(filterObj, self.selectionModel.allMatter, '1');
            ...
            function getAllMatterList(data) {
                self.dataReceived = false;
                //getMatterList(10000);
                getMatterList('All', 'calDays');
            }

            function getNextMatterList() {
                var filterObj = upcomingSOLsReportHelper.getFiltersObj(self.viewModel.filters);
            ...
            function downloadMatters() {
                var filterObj = upcomingSOLsReportHelper.getFiltersObj(self.viewModel.filters, 10000);
                ...
                reportFactory.downLoadUpcomingSOLs(filterObj, self.viewModel.filters, '1', self.selectionModel.allMatter)
                    .then(function (response) {
                        utils.downloadFile(response.data, "Upcoming_SOLs_Report.xlsx", response.headers("Content-Type"));
                    })
            }
            ...
            function print() {
                ...
                reportFactory.printSOLsMatter(self.viewModel.matters, printObjFilter);
            };
...
(function () {
    angular.module('cloudlex.report')
        .factory('upcomingSOLsReportHelper', upcomingSOLsReportHelper);

    upcomingSOLsReportHelper.$inject = ['$filter', 'reportConstant', '$http'];

    function upcomingSOLsReportHelper($filter, reportConstant, $http) {
        return {
        ...
        function setModifiedDisplayDate(matterList) {
            _.forEach(matterList.data, function (matter) {
                ...
                matter.matter_type_name = matter.matter.type;
                matter.matter_sub_type_name = matter.matter.sub_type;
                matter.category_name = matter.matter.category;
                matter.status_name = matter.matter.status;
                matter.sub_status_name = matter.matter.sub_status;

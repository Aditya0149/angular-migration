(function (angular) {
    angular.module('cloudlex.report').
        controller('expenseFilterCtrl', expenseFilterCtrl);

    expenseFilterCtrl.$inject = ['$scope', '$modalInstance', 'notification-service', 'matterDetailsService', 'matterFactory', 'expenseHelper', 'filter', 'tags'];

    function expenseFilterCtrl($scope, $modalInstance, notificationService, matterDetailsService, matterFactory, expenseHelper, filter, tags) {
        var self = this;
        var matters = [];
        self.apply = apply;
        ...
        function apply(filter) {
            ...
            //if (angular.isDefined(filter.matterids)) {
            // if(utils.isNotEmptyVal(filter.matterid.matterid) || utils.isEmptyVal(filter.matterid)){
        ...
        function searchMatters(matterName) {
            ...
            if (matterName) {
                return matterFactory.searchMatters(matterName).then(
                    function (response) {
                        self.searchMatterList = response.data; //US#8644 Expense Billing updates

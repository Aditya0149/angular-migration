(function () {
    ...
    angular
        ...
        .controller('officeDocViewCtrl', officeDocViewCtrl);

    officeDocViewCtrl.$inject = ['documentsDataService', 'notification-service', '$state', 'documentsConstants', '$rootScope'];

    function officeDocViewCtrl(documentsDataService, notificationService, $state, documentsConstants, $rootScope) {
        var self = this;
        var docdetails = {};
        var url, mode;
        docdetails = documentsDataService.getDocumentInfo();
        if (utils.isEmptyObj(docdetails)) {
            docdetails = JSON.parse(localStorage.getItem("selDocumentInfo"));
        }
        ...
        if (path.indexOf("file=") == -1) {
            ...
            documentsDataService.getDocumentDetails(self.matterId, self.documentId)

angular.module('intake.dashboard')
    .factory('intakeDashboardHelper', intakeDashboardHelper);

intakeDashboardHelper.$inject = ['globalConstants'];
function intakeDashboardHelper(globalConstants) {
    ...
    return {
        ...
        setDonutChart: _setDonutChart,
        setAvgValDonutChart: _setAvgValDonutChart,
    ...
    function _getInitData() {
        ...
        var initData = {
            matterTrends: ["New vs Accepted Leads"],
            matterTrendFilters: [{ key: '3M', value: '3M', val: 3 },
            { key: '6M', value: '6M', val: 6 },
            { key: '1Y', value: '1Y', val: 12 }],
    ...
    function _setMatterTrendChartData(data) {
        var updatedData = [];
        var oneYearArrayFromThisMonth = _getOneYearFromThisMonthArray(data.duration);

        _.forEach(oneYearArrayFromThisMonth, function (month) {
            var newMonthFound = _.find(data.newLeads, function (item) { return item.date === month });
            var acceptedMonthFound = _.find(data.acceptedLeads, function (item) { return item.date === month });
            var newCount = newMonthFound ? parseInt(newMonthFound.count) : 0;
            var acceptedCount = acceptedMonthFound ? parseInt(acceptedMonthFound.count) : 0;

            updatedData.push({ date: month, newCount: newCount, acceptedCount: acceptedCount });
        })
        return updatedData;
    }

    function _getOneYearFromThisMonthArray(duration) {
        var now = moment();
        ...
        //     var dateString = date.format('MMMM YYYY');
        ...
        for (var i = 1; i <= duration; i++) {
            var dateString = today.format('MMMM YYYY');
            monthsArray.push(dateString);
            today = today.subtract(1, 'months')
        }

        //console.log("current= "+monthsArray);
        var updatedMonthsArray = monthsArray.reverse();
        updatedMonthsArray = updatedMonthsArray.slice(0, duration);
        monthsArray = updatedMonthsArray.reverse();
        //console.log("updated= "+monthsArray);
        return monthsArray;
    };
    ...
    function _setAvgValDonutChart(bindTo, data) {
        var values = [];
        ...
        _.forEach(keys, function (key, index) {
            var valForKey = [];
            if (allZero) {
                allZero = data[key] > 0 ? false : true;
            }
            valForKey.push(data[key]);
            valForKey.unshift(getLegendText(key));
            values.push(valForKey);
        });

        values = allZero ? [] : values;
    ...
    function _setDonutChart(bindTo, data) {
        var values = _getDonutValues(data);
        var allZero = true;

        _.forEach(values, function (key, index) {
            if (allZero) {
                allZero = key[1] > 0 ? false : true;
            }
        });
    ...
    function _setAreaChart(bindTo, chartName, data) {
        ...
        var allZero = true;
        _.forEach(data, function (rec, index) {
            if (allZero) {
                allZero = ((rec.newCount > 0) || (rec.acceptedCount > 0)) ? false : true;
                //allZero = true;
            }
        });
        ...
        var colVal = [];
        if (!allZero) {
            colVal.push(categories);
            colVal.push(values.values);
            colVal.push(values.values2);
        }
    ...
    function _getDonutValues(data) {
        var values = [];
        var donutForUniqueValues = getKeyValuePair(data, "typeName", "count");
    ...
    function _getValues(chartName, data) {
        ...
        var values = [];
        _.forEach(data, function (item) {
            val.values.push(item.newCount);
            val.values2.push(item.acceptedCount);
        });

        val.values.unshift("New Leads");
        val.values2.unshift("Accepted Leads");
        return val;
    }
    ...
    function _getCategoriesForAreaChart(data) {
        var categories = [];
        _.forEach(data, function (item) {
            var date = moment(item.date, 'MMMM YYYY');
    ...
    function _setMatterTypeAreaChart(matterType, filterKey, data, chartInfo, unloadType) {
        ...
        var valuesForChart = _getValues(matterType, data).values;
    ...
    function _arrangeMatterAgeData(data) {
        ...
        var order = ["New Case", "Litigation", "Discovery", "Trial", "Motion"];
        var orderedData = [];

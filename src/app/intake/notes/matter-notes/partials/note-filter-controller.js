...
(function () {
    ...
    angular
        .module('intake.notes')
        .controller('IntakeNoteFiltersCtrl', NoteFiltersController);

    NoteFiltersController.$inject = ['intakeNotesDataService', '$scope', '$modalInstance', 'matterID', 'categories', 'users', 'selectedFilters', 'currentDateFilter', 'tags', 'contactFactory', 'matterFactory', 'globalConstants'];

    function NoteFiltersController(intakeNotesDataService, $scope, $modalInstance, matterID, categories, users, selectedFilters, currentDateFilter, tags, contactFactory, matterFactory, globalConstants) {
        $scope.categories = [];
        ...
        $scope.$watch(function () {
            if ($scope.currentFilter && !utils.isEmptyObj($scope.currentFilter)) {
                if ((angular.isDefined($scope.currentFilter.catFilter) && $scope.currentFilter.catFilter.length > 0) || utils.isNotEmptyVal($scope.currentFilter.end) ||
                    utils.isNotEmptyVal($scope.currentFilter.start) || (angular.isDefined($scope.currentFilter.uidFilter) && $scope.currentFilter.uidFilter.length > 0) ||
            ...
            if (utils.isNotEmptyVal(name)) {
                ...
                matterFactory.getContactsByName(postObj, true)
                    .then(function (response) {
                        ...
                        // self.contact = data.data.contacts;
                        // self.contactList1 = [];
                        // _.forEach(self.contact, function (item) {
        ...
        $scope.applyFilters = function () {
            ...
            intakeNotesDataService.getFilteredNotes(matterID, $scope.currentFilter, intakeNotesDataService.customPageNum)
                .then(function (data) {
                    $modalInstance.close({ filterData: data.notes, totalCount: data.totalCount, filters: $scope.currentFilter, currentDateFilter: $scope.currentDateFilter });
                });
        };

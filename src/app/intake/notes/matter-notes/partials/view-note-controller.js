...
(function () {
    ...
    angular
        .module('intake.notes')
        .controller('IntakeViewNoteCtrl', ViewNoteController);

    ViewNoteController.$inject = ['intakeNotesDataService', '$scope', '$modalInstance', 'note', 'categories', 'globalConstants', 'mailboxDataService', 'mailboxDataServiceV2', 'masterData', '$state', 'matterFactory', 'contactFactory', 'notesDataService','$filter'];

    function ViewNoteController(intakeNotesDataService, $scope, $modalInstance, note, categories, globalConstants, mailboxDataService, mailboxDataServiceV2, masterData, $state, matterFactory, contactFactory, notesDataService, $filter) {
        $scope.isIntake = true;
        $scope.note = angular.copy(note.note);
            ...
            if (utils.isNotEmptyVal(name)) {
                ...
                matterFactory.getContactsByName(postObj, true)
                    .then(function (response) {
                        ...
                        // var allLinkedContactId = _.pluck($scope.noteToEdit.linked_contact_temp,'contact_id');
                        var allLinkedContactId = $scope.noteToEdit.linked_contact_temp;
                ...
                if (angular.isDefined($scope.noteToEdit.linked_contact_temp)) {
                    // $scope.contactList1 = [];
                    var tempLinkedContact = [];
                    _.forEach($scope.noteToEdit.linked_contact_temp, function (item) {
                        _.forEach($scope.contactList1, function (it) {
                            if (item == it.contact_id) {
                                tempLinkedContact.push(it);
                            }
                        })
                    });
                    $scope.contactList1 = [];
        ...
        $scope.migrate = localStorage.getItem('Migrated');
        ...
        if (note.type == 'sidebar') {
            if (note.noteCategory.notecategory_id == 1002) {
                intakeNotesDataService.sidebarComment(note.note_id)
                    .then(function (response) {
        ...
        */
        ...
        function edit() {
            ...
            //     var linked_contact_info = [];
            //     linked_contact_info.push(item.contact_id);
            //     linked_contact_info.push({'firstname':item.first_name});
            //     linked_contact_info.push({'lastname':item.last_name});
            //     $scope.noteToEdit.linked_contact_temp = linked_contact_info;
            // });
            var list = angular.copy($scope.note.linked_contact);
            $scope.noteToEdit['linked_contact_temp'] = [];
            _.forEach(list, function (item) {
                $scope.noteToEdit['linked_contact_temp'].push(item.contact_id.toString());
            });
        ...
        $scope.editNote = function (note) {
            ...
            intakeNotesDataService.editNote(note)
                .then(function (res) {
        ...
        $scope.printNote = function (note, sidebar) {
            var intake_name = note.matter_name;
        ...
        function emailPrintView(note) {
            ...
            html += " <small>Intake Name:" + utils.removeunwantedHTML(note.matter_name) + " </small>" + "</br>";
            html += "<small>Added By: " +
                utils.removeunwantedHTML(note.user.first_name) + ' ' + utils.removeunwantedHTML(note.user.last_name) + "|" + getFormattedDate(note.created_date)
                + "</small>" + "</br>";
            ...
            _.forEach(note.emailMessages, function (mail, key) {
                ...
                html += "</tr>";
                html += "<tr>";
                html += "<td>Date: " + mail.createddate + "</td>";
                html += "</tr>";

                html += "<tr>";
                html += "<td style='padding:10px 0;'>Sub: " + utils.removeunwantedHTML(mail.subject) + "</td>";
                html += "</tr>";



                html += "<tr>";
                html += "<td>" + utils.removeunwantedHTML(mail.message) + "</td>";
                html += "</tr>";
                html += "</table>";
        ...
        $scope.doDeleteNote = function () {
            intakeNotesDataService.deleteNote($scope.note.note_id)
                .then(function (response) {

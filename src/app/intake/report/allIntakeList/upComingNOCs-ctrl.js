(function () {
    ...
    angular.module('intake.report').controller('IntakeupcomingNOCsCtrl',
        ['$q', 'intakeReportFactory', 'upcomingIntakeNOCsReportHelper', '$modal',
            function ($q, intakeReportFactory, upcomingIntakeNOCsReportHelper, $modal) {
                ...
                self.init = function () {
                    var viewModel = sessionStorage.getItem("intakeupcomingNOCViewModel");
                    var selectionModel = sessionStorage.getItem("intakeupcomingNOCSelectionModel");
                    ...
                    self.clxGridOptions = {
                        headers: upcomingIntakeNOCsReportHelper.getGridHeaders(),
                        selectedItems: []
                    };
                }
                ...
                function persistFilters() {
                    ...
                    viewModel.matters = [];
                    sessionStorage.setItem("intakeupcomingNOCViewModel", JSON.stringify(viewModel));
                    sessionStorage.setItem("intakeupcomingNOCSelectionModel", JSON.stringify(selectionModel));
                }
                ...
                function applyFilterLazyLoading() {
                    ...
                    self.viewModel.filters.lastMatterId = _.last(self.viewModel.matters)['intakeId'];
                    getNextMatterList();
                };
                ...
                function getMatterList(getAll, info) {
                    ...
                    var filterObj = upcomingIntakeNOCsReportHelper.getFiltersObj(self.viewModel.filters, getAll, self.selectionModel.allMatter);
                    persistFilters();
                    ...
                    promesa.then(function (data) {
                        ...
                        upcomingIntakeNOCsReportHelper.setModifiedDisplayDate(matterList);
                        self.viewModel.matters = solData.events;
                        self.showPager = self.viewModel.matters.length < self.total;
                        self.tags = upcomingIntakeNOCsReportHelper.generateTags(self.viewModel.filters);
                ...
                function getNextMatterList() {
                    var filterObj = upcomingIntakeNOCsReportHelper.getFiltersObj(self.viewModel.filters, '', self.selectionModel.allMatter);
                    ...
                    promesa.then(function (data) {
                        var solData = angular.copy(data);
                        processData(solData);
                        upcomingIntakeNOCsReportHelper.setModifiedDisplayDate(solData);
                        _.forEach(solData.events, function (item) {
                ...
                function downloadMatters() {
                    ...
                    var filterObj = upcomingIntakeNOCsReportHelper.getFiltersObj(self.viewModel.filters, 10000, self.selectionModel.allMatter);
                    intakeReportFactory.downLoadUpcomingSOLs(filterObj, self.viewModel.filters)
                        .then(function (response) {
                            utils.downloadFile(response.data, "Upcoming_Intake_NOCs_Report", response.headers("Content-Type"));
                ...
                function print() {
                    ...
                    intakeReportFactory.printNOCsIntake(self.viewModel.matters, printObjFilter);
                };
...
(function () {
    angular.module('intake.report')
        .factory('upcomingIntakeNOCsReportHelper', upcomingIntakeNOCsReportHelper);

    upcomingIntakeNOCsReportHelper.$inject = ['$filter', 'reportConstant'];
    function upcomingIntakeNOCsReportHelper($filter, reportConstant) {
        return {

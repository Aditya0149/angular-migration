(function () {
    ...
    angular.module('intake.report').controller('IntakeValuationCtrl', ['$q', 'intakeReportFactory', '$filter', '$state', '$modal', 'masterData',
        'intakeValuationReportHelper',
        function ($q, intakeReportFactory, $filter, $state, $modal, masterData,
            intakeValuationReportHelper) {
            ...
            { order: 'ASC', param: "case_value_asc", name: 'Case value ASC', value: 'intake_amount' },
            { order: 'DESC', param: "case_value_desc", name: 'Case value DESC ', value: 'intake_amount DESC' },
            ...
            self.init = function () {
                ...
                var selectionModel = sessionStorage.getItem("intakeValuationListReportSelectionModel");
                var viewModel = sessionStorage.getItem("intakeValuationListReportViewModel");
                ...
                if (utils.isNotEmptyVal(selectionModel) || utils.isNotEmptyVal(viewModel)) {
                    try {
                        ...
                        self.tags = intakeValuationReportHelper
            ...
            function persistFilters(filters) {
                ...
                viewModel.allIntake = [];
                sessionStorage.setItem("intakeValuationListReportSelectionModel", JSON.stringify(selectionModel));
                sessionStorage.setItem("intakeValuationListReportViewModel", JSON.stringify(viewModel));
            }
            ...
            function applySetFilters() {
                ...
                self.clxGridOptions = {
                    headers: intakeValuationReportHelper.getGridHeaders(),
                    selectedItems: []
                };
            }
            ...
            function resetAllFilter() {
                ...
                self.viewModel.userFilter.lowlimit = '';
                self.viewModel.userFilter.maxlimit = '';
            ...
            self.filterAllIntakeReport = function () {
                ...
                var modalInstance = $modal.open({
                    templateUrl: 'app/intake/report/allIntakeList/filterPopUp/intakeValuation/intakeValuationFillter.html',
                    controller: 'IntakeValuationFilterCtrl as intakeValuationFilter',
                    backdrop: 'static',
                    keyboard: false,
                    resolve: {
                        params: function () {
                            return {
                                ...
                                filterObj: self.viewModel.filters,
                ...
                modalInstance.result.then(function (filterObj) {
                    ...
                    self.viewModel.filters.s = utils.isEmptyObj(filterObj.filters) || _.isNaN(filterObj.filters) ? undefined : filterObj.filters.s;
                    ...
                    self.viewModel.name = filterObj.dateFilter;
                    self.displayMoreButton = true;
                    self.displayALlButton = true;
            ...
            function setUserFilterForFilter(userFilterObj) {
                ...
                userFilterObj.lowlimit = utils.isNotEmptyVal(self.viewModel.userFilter.lowlimit) ?
                    self.viewModel.userFilter.lowlimit : null;

                userFilterObj.maxlimit = utils.isNotEmptyVal(self.viewModel.userFilter.maxlimit) ?
                    self.viewModel.userFilter.maxlimit : null;
            ...
            self.applyMultiSelectFilter = function (tagcancelled) {
                ...
                self.tags = intakeValuationReportHelper.createFilterTags(self.viewModel.filters, self.viewModel.masterList, self.viewModel.userFilter, self.viewModel.userList, self.viewModel.dateFilter);
                self.getIntakeList();
                self.viewModel.page = "GRIDVIEW";
            };
            ...
            self.tagCancelled = function (cancelled) {
                if (timeFilters.indexOf(cancelled.id) > -1) {
                    ...
                    if (cancelled.type === 'referebyFilter') {
                        ...
                        self.viewModel.userFilter['campaign'] = '';
                    } else if (cancelled.type === 'MatterValuationRange') {
                        self.viewModel.userFilter['lowlimit'] = '';
                        self.viewModel.userFilter['maxlimit'] = '';
            ...
            self.applySortByFilter = function (sortBy) {
                ...
                } else if (sortBy.param == "case_value_asc") {
                    self.viewModel.filters.sortby = "intake_amount";
                } else if (sortBy.param == "case_value_desc") {
                    self.viewModel.filters.sortby = "intake_amount DESC";
                }
                self.viewModel.filters.pagenum = 1;
                self.getIntakeList();
            }
            ...
            self.getIntakeList = function (getAll) {
                ...
                var filterObj = intakeValuationReportHelper.getFiltersObj(self.viewModel.filters, getAll, self.viewModel.masterList);


                var promesa = intakeReportFactory.getIntakeVaulationReportList(filterObj);
                promesa.then(function (response) {
                    ...
                    intakeValuationReportHelper.setModifiedDisplayDate(intakeList);
            ...
            function processData(response) {
                _.forEach(response.intakeData, function (item) {
                    ...
                    item.migrationDate = (item.migrationDate == 0) ? "-" : moment.unix(item.migrationDate).format('DD MMM YYYY');
                    ...
                    if (item.intakeAmount == null) {
                        item.intakeAmount = utils.isNotEmptyVal(item.intakeAmount) ? currencyFormat(parseFloat(item.intakeAmount).toFixed(2)) : " ";
                    } else {
                        item.intakeAmount = utils.isNotEmptyVal(item.intakeAmount) ? currencyFormat(parseFloat(item.intakeAmount).toFixed(2)) : $filter('currency')(0, '$', 2);
                    }
            ...
            function setUserFilter() {
                ...
                self.viewModel.filters.lowlimit = utils.isNotEmptyVal(self.viewModel.userFilter.lowlimit) ?
                    self.viewModel.userFilter.lowlimit : '';

                self.viewModel.filters.maxlimit = utils.isNotEmptyVal(self.viewModel.userFilter.maxlimit) ?
                    self.viewModel.userFilter.maxlimit : '';

                // self.viewModel.filters.s = utils.isNotEmptyVal(self.viewModel.filters.s) ?
                //     self.viewModel.filters.s : '';

                // self.viewModel.filters.e = utils.isNotEmptyVal(self.viewModel.filters.e) ?
                //     self.viewModel.filters.e : '';
            ...
            function getNextMatterList() {
                ...
                setUserFilter();
                var filterObj = intakeValuationReportHelper.getFiltersObj(self.viewModel.filters, '', self.viewModel.masterList);
                var promesa = intakeReportFactory.getIntakeVaulationReportList(filterObj);
            ...
            self.downloadMatters = function () {
                ...
                var filterObj = intakeValuationReportHelper.getFiltersObj(self.viewModel.filters, '', self.viewModel.masterList);
                filterObj.requestType = "valuationReport";
                ...
                intakeReportFactory.downloadMatters(filterObj)
                    .then(function (response) {
                        utils.downloadFile(response.data, "Intake_Valuation_Report", response.headers("Content-Type"));
            ...
            self.print = function () {
                ...
                promesa.then(function (response) {
                    ...
                    intakeReportFactory.printIntakeValuation(data, filterDisplayObj);
                }, function (reason) { });
            };
            ...
            function setFilterDisplayObj() {
                ...
                setEstimatedCaseValueForPrint(filterObj);
            ...
            function setEstimatedCaseValueForPrint(filterObj) {
                if ((utils.isEmptyVal(self.viewModel.filters.lowlimit) && utils.isEmptyVal(self.viewModel.filters.maxlimit))) {
                    var policy = "";
                } else {
                ...
                var caseValue = currencyFormat(self.viewModel.filters.lowlimit) + " to " + currencyFormat(self.viewModel.filters.maxlimit);
                filterObj.estimateValue = {
...
(function () {
    angular.module('intake.report')
        .factory('intakeValuationReportHelper', intakeValuationReportHelper);
    intakeValuationReportHelper.$inject = [];

    function intakeValuationReportHelper() {
        return {
        ...
        function getGridHeaders() {
            return [
                ...
                },
                {
                    ...
                    displayName: 'Estimated Case Value',
                    dataWidth: 10
        ...
        function getFiltersObj(filters, getAll, masterList) {
            ...
            formattedFilters.minimumAmount = utils.isNotEmptyVal(filters.lowlimit) ? parseFloat(filters.lowlimit) : null;
            formattedFilters.maximumAmount = utils.isNotEmptyVal(filters.maxlimit) ? parseFloat(filters.maxlimit) : null;
            //leadNameFilter
        ...
        function createFilterTags(filters, masterList, userFilters, userList, dateFilter) {
            ...
            if (utils.isNotEmptyVal(userFilters.lowlimit) || utils.isNotEmptyVal(userFilters.maxlimit)) {
                var limit_min = utils.isNotEmptyVal(userFilters.lowlimit) ? currencyFormat(userFilters.lowlimit) : '';
                var limit_max = utils.isNotEmptyVal(userFilters.maxlimit) ? currencyFormat(userFilters.maxlimit) : '';
                var policy;
                if ((utils.isEmptyVal(limit_max) && utils.isEmptyVal(limit_min))) {
                    policy = "";
                } else {
                    policyLimit_max = utils.isNotEmptyVal(limit_max) ? limit_max : "-";
                    policyLimit_min = utils.isNotEmptyVal(limit_min) ? limit_min : "-";
                    policy = policyLimit_min + '/' + policyLimit_max
                }
                var tagObj = {
                    type: 'MatterValuationRange',
                    value: 'Estimate case value range: ' + policy
                };
                tags.push(tagObj);
            }

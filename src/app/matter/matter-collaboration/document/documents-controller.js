...
(function () {
    ...
    angular
        ...
        .controller('MatterCollaborationDocumentsCtrl', MatterCollaborationDocumentsController);

    MatterCollaborationDocumentsController.$inject = ['$scope', '$window', 'documentsDataService', '$stateParams', 'documentsConstants', 'collaborationDocumentListHelper', 'mailboxDataService',
        'notification-service', '$modal', 'routeManager', 'matterFactory', 'contactFactory', '$rootScope', 'globalConstants', 'masterData', '$state', 'allPartiesDataService', 'loginDatalayer', 'practiceAndBillingDataLayer'
    ];

    function MatterCollaborationDocumentsController($scope, $window, documentsDataService, $stateParams, documentsConstants, collaborationDocumentListHelper, mailboxDataService,
        notificationService, $modal, routeManager, matterFactory, contactFactory, $rootScope, globalConstants, masterData, $state, allPartiesDataService, loginDatalayer, practiceAndBillingDataLayer) {
        var self = this,
            initLimit = 15;
        ...
        selectDocument = collaborationDocumentListHelper.getSelectedDocument();
        ...
        function getOfficeOnlineStatus() {
            documentsDataService.getOfficeStatus()
                .then(function (response) {
                    ...
                    self.clxGridOptions = {
                        headers: collaborationDocumentListHelper.getGridHeaders(self.isGlobalDocs, self.officeOnlineStat),
                        selectedItems: []
                    };
                    getDocuments();
                }, function (error) {
        ...
        self.viewCommentMedicalInfo = function (selectedItem) {
            var modalInstance = $modal.open({
                ...
                controller: 'collaborateMatterViewCommentDocumentCtrl as viewCommentInfo',
                keyboard: false,
                size: 'lg',
        ...
        function composeNotesMail() {
            ...
            if (self.isGlobalDocs) {
                ...
                /*
                 * set document list 
                 * rootScope: composeEmail.matterdocs 
                 */
                ...
                documentsDataService.getDocumentsList(true, self.matterId, false, '1', self.pageSize, '1', 'asc', '').then(function (data) {
                    ...
                    getMatterContact(self.matterId, function (contacts) {
                        $rootScope.updateComposeMailMsgBody(self.composeEmail, selectedMatter, data.data, self.clxGridOptions.selectedItems, 'matterRecipents', contacts);
                    });
                }, function (reason) {
        ...
        function setFilters() {
            ...
            if (utils.isNotEmptyVal(displayObj)) {
                try {
                    ...
                    //Ends here
                } catch (e) { self.display = collaborationDocumentListHelper.docDisplayOptions(self.isGlobalDocs); }
            } else {
                self.display = collaborationDocumentListHelper.docDisplayOptions(self.isGlobalDocs);
                if (self.isGlobalDocs) {
        ...
        function isDocSelected(doc) {
            if (self.clxGridOptions != undefined) {
                self.display.documentSelected[doc.doc_id] =
                    collaborationDocumentListHelper.isDocumentSelected(self.clxGridOptions.selectedItems, doc);
        ...
        self.toggleFilterPage = function () {
            ...
            modalInstance.result.then(function (selectedItem) {
                collaborationDocumentListHelper.setFilterObj(self.display.filters, selectedItem.filters);
                self.filterddocs = true;
        ...
        self.opendocumentView = function (doc, isGlobalDoc, $event, actionType) {
            ...
            var modalInstance = $modal.open({
                ...
                controller: "CollaborateMatterDocumentsPopupViewCtrl as docpopupviewCtrl",
                size: 'lg',
                backdrop: 'static',
                keyboard: false,
                windowClass: 'modalLargeDialog',
        ...
        $scope.$watch(function () {
            localStorage.setItem('matterCollaborationDocuments', JSON.stringify(self.clxGridOptions.selectedItems))
            $rootScope.matterCollaborationDocumentsCount = self.clxGridOptions.selectedItems.length;
        });

        // checkMatterCollaborationDocumentsCount();
        // function checkMatterCollaborationDocumentsCount(){
        //     $scope.$watch(function () {
        ...
        self.matterCollaborationDocPermissionToggle = matterCollaborationDocPermissionToggle;

        function matterCollaborationDocPermissionToggle(flag) {
            localStorage.setItem('matterCollaborationDocPermission', flag);
        }
...
(function () {
    ...
    angular
        .module('cloudlex.matter')
        .controller('CollaborateMatterDocumentsPopupViewCtrl', CollaborateMatterDocumentsPopupViewCtrl);

    CollaborateMatterDocumentsPopupViewCtrl.$inject = ['$scope', '$rootScope', 'documentsDataService', 'documentsConstants', 'notification-service', '$state', 'masterData', '$modalInstance', 'docdetails', '$modal', 'matterFactory'];

    function CollaborateMatterDocumentsPopupViewCtrl($scope, $rootScope, documentsDataService, documentsConstants, notificationService, $state, masterData, $modalInstance, docdetails, $modal, matterFactory) {
...
(function () {
    angular
        .module('cloudlex.matter')
        .controller('collaborateMatterViewCommentDocumentCtrl', collaborateMatterViewCommentDocumentCtrl);

    collaborateMatterViewCommentDocumentCtrl.$inject = ['$rootScope', '$modalInstance', 'viewCommentInfo'];

    function collaborateMatterViewCommentDocumentCtrl($rootScope, $modalInstance, viewCommentInfo) {
        var vm = this;
...
(function () {
    angular.module('cloudlex.matter')
        .factory('collaborationDocumentListHelper', collaborationDocumentListHelper);

    function collaborationDocumentListHelper() {

(function () {
    ...
    angular
        ...
        .controller('GlobalNotesCtrl', GlobalNotesController);

    GlobalNotesController.$inject = ['$rootScope', '$scope', '$timeout', '$filter', 'notesDataService', '$modal',
        'notification-service', '$state', 'globalNoteHelper', 'contactFactory',
        'globalConstants', 'mailboxDataService', 'mailboxDataServiceV2', 'masterData', 'modalService', 'matterFactory', '$q'];

    function GlobalNotesController($rootScope, $scope, $timeout, $filter, notesDataService, $modal,
        ...
        function preInit() {	//Get notes and categories
            init();
            var currentNotesView = sessionStorage.getItem('global-note-view');
        ...
        $rootScope.$on("noteAdded", function () {
            var type = sessionStorage.getItem('globalNotesType');
            if (type == 'allNotes') {
                self.allNotes();
        ...
        function searchRetain() {
            ...
            sessionStorage.setItem("globalNotesText", globalNotesText);
        ...
        function resetNotes() {
            ...
            sessionStorage.setItem("globalNotesFilters", JSON.stringify({}));
            sessionStorage.setItem("globalNotesText", '');
            init();
            var type = sessionStorage.getItem('globalNotesType');
            if (type == 'allNotes') {
                self.allNotes();
        ...
        function init() {
            ...
            self.currentView = utils.isNotEmptyVal(sessionStorage.getItem("global-note-grid-view")) ? sessionStorage.getItem("global-note-grid-view") : "LIST";
            ...
            self.currentFilter = {
                catFilter: [],
                uidFilter: [],
                tab: (sessionStorage.getItem('globalNotesType') == "myNotes") ? 'myglobalnotes' : 'allglobalnotes',
                impFilter: 0,
                start: '',
                end: '',
                s_custom: '',
            ...
            //Bug#7326
            var globalNotesText = sessionStorage.getItem("globalNotesText");
        ...
        function setSelectedFilters() {
            var selectedFilters = sessionStorage.getItem("globalNotesFilters");
            ...
            self.matterId = utils.isEmptyVal(self.selectedFilters.matterid) ?
                'global' : self.selectedFilters.matterid.matterid;
        }

        function setExistingFilters() {
            var existingFilters = sessionStorage.getItem("globalNotesFilters");
        ...
        function myNotes() {
            ...
            sessionStorage.setItem('global-note-view', "my");
            if (self.globalNotesGrid && self.globalNotesGrid.selectedItems) {
                self.globalNotesGrid.selectedItems = [];
            }
            sessionStorage.setItem('globalNotesType', "myNotes");
            self.notesList = [];
            notesDataService.getFilteredMyNotes(self.matterId, self.selectedFilters, self.currentPageNum)
                .then(function (data) {
        ...
        self.allNotes = function () {
            ...
            sessionStorage.setItem('global-note-view', "all");
            if (self.globalNotesGrid && self.globalNotesGrid.selectedItems) {
                self.globalNotesGrid.selectedItems = [];
            }
            sessionStorage.setItem('globalNotesType', "allNotes");
            self.notesList = [];
            notesDataService.getFilteredAllNotes(self.matterId, self.selectedFilters, self.currentPageNum)
                .then(function (data) {
                    self.isDataAvailable = true;
        ...
        function getNoteCategories() {
            return notesDataService.getNotesCategories()
                .then(function (data) {
        ...
        self.setCurrentView = function (viewName) {
            ...
            sessionStorage.setItem("global-note-grid-view", self.currentView);
            self.globalNotesGrid.selectedItems = [];
            setGridHeight();
        };
        ...
        self.openFilters = function () {
            ...
            if (self.isFilterView) {
                sessionStorage.setItem("globalNotesFilters", JSON.stringify(self.selectedFilters));
            }
            else {
                setExistingFilters();
            }
        ...
        self.applyCatFilters = function (convCatFilter) {
            ...
            notesDataService.getFilteredNotes('global', self.selectedFilters, self.currentPageNum)
                .then(function (data) {
                    if (data && data.notes) {
                        self.notesList = data.notes;
                    }
        ...
        self.applyFilters = function (currentFilter) {
            ...
            if (_.isUndefined(currentFilter.matterid) || currentFilter.matterid == "") {
                ...
                self.matterId = currentFilter.matterid.matterid;
            }
            self.selectedFilters = angular.copy(currentFilter);

            //persist notes filter
            self.selectedFilters.currentDateFilter = self.addedOnVal;
            self.selectedFilters.tab = sessionStorage.getItem('globalNotesType') == "myNotes" ? 'myglobalnotes' : 'allglobalnotes'
        ...
        self.doDeleteNote = function () {
            notesDataService.deleteNote(self.note.note_id)
                .then(function (response) {
                    self.deleteMode = false;
                    self.isOpenNotesView = false;
                    notificationService.success('Note deleted successfully.');
        ...
        function exportGlobalNote() {
            ...
            notesDataService.exportMatterNotes(filter, self.matterId)
                .then(function (response) {
                    utils.downloadFile(response.data, "GlobalMatterNotes.xlsx", response.headers("Content-Type"));
...
(function () {
    ...
    angular
        .module('cloudlex.notes')
        .controller('ViewGlobalNoteCtrl', ViewGlobalNoteCtrl);

    ViewGlobalNoteCtrl.$inject = ['notesDataService', '$scope', '$modalInstance', 'globalConstants', 'NoteData', 'masterData', 'notification-service', '$state', 'matterFactory', 'contactFactory', '$filter'];

    function ViewGlobalNoteCtrl(notesDataService, $scope, $modalInstance, globalConstants, NoteData, masterData, notificationService, $state, matterFactory, contactFactory, $filter) {
        var self = this;
            ...
            if (utils.isNotEmptyVal(name)) {
                ...
                matterFactory.getContactsByName(postObj, true)
                    .then(function (response) {
                        ...
                        contactFactory.setDataPropForContactsFromOffDrupalToNormalContact(data);
                        contactFactory.setNamePropForContactsOffDrupal(data);
                        var allLinkedContactId = _.pluck(self.note.linked_contact_temp, 'contact_id');
                        var responseData = angular.copy(data);
                        _.forEach(responseData, function (item, index) {
                            _.forEach(allLinkedContactId, function (it) {
                                if (item.contact_id == it) {
        ...
        if (self.note.type == 'sidebar') {
            if (self.note.noteCategory.notecategory_id == 1002) {
                notesDataService.sidebarComment(self.note.note_id)
                    .then(function (response) {
        ...
        function edit() {
            ...
            if (self.notesPermissions[0].E == 0) {
                notificationService.error(globalConstants.accessMgmtMessage + "edit notes");
                return;
            }
        ...
        self.deleteNote = function () {
            if (self.notesPermissions[0].D == 0) {
                notificationService.error(globalConstants.accessMgmtMessage + "delete notes");
                return;
            }
...
(function (angular) {
    angular
        .module('cloudlex.notes')
        .factory('globalNoteHelper', globalNoteHelper);

    globalNoteHelper.$inject = ['notesDataService', 'globalConstants', 'masterData', '$filter'];

    function globalNoteHelper(notesDataService, globalConstants, masterData, $filter) {
        ...
        function _getFiltersForPrint(filter, user, eventClick) {
            ...
            if (utils.isNotEmptyVal(filter.matterid)) {
                matterName = utils.removeunwantedHTML(filter.matterid.name);
            }
        ...
        function _getPrintPage(filters, noteList) {
            var html = "<html><title>Matter Notes</title>";
            ...
            html += "<th width='15%' style='border:1px solid #e2e2e2;background-color:#E9EEF0!important;-webkit-print-color-adjust:exact; border-collapse:collapse; padding:5px; '>Matter Name</th>";
        ...
        function spliceFilter(cancelled, selectedFilters) {
            ...
            } else if (cancelled.type == 'showIntakeEvents') {
                delete selectedFilters.showIntakeEvents;
            } else if (cancelled.type == 'showMatterEvents') {
                delete selectedFilters.showMatterEvents;
            }
        }
